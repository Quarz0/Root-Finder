...
def bisection(xl, xu, func, iterations=50, eps=0.00001):
    ...
    for i in xrange(iterations):
        ...
        if evaluateFunc(func, xl) * evaluateFunc(func, xr) < 0: xu = xr;
        else: xl = xr;
    ...
    table = Table(['Step', 'xl', 'f(xl)', 'xu', 'f(xu)', 'xr', 'Ea (%)'], iterationRows)

    return ResultSet(table, xr, calcPrecision(ea), executionTime, i, [sympy.lambdify('x', expr, 'numpy')], vLines = boundaryLines)
